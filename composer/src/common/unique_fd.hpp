// unique_fd — move-only RAII wrapper around an int file descriptor.
//
...
//
// Usage:
//   vn::base::unique_fd fd(::open(path, O_RDONLY | O_CLOEXEC));
//   if (!fd) return -1;            // operator bool checks fd >= 0
//   ssize_t n = ::read(fd.get(), buf, len);
//   int raw = fd.release();         // transfer to C API (e.g. struct field)
//
// Return-by-value is fine (RVO; otherwise the move constructor takes over).
//
// ─────────────────────────────────────────────────────────────────────────
...
// A move-only RAII wrapper around `int` file descriptors. Closes the fd in
// the destructor via the supplied Closer (defaults to ::close). Holds -1
// when empty.
//
...
template <typename Closer> class unique_fd_impl {
  public:
    ...
    // Close the held fd (if any) and take ownership of `new_value`.
    ...
    // Transfer ownership of the fd back to the caller. Caller is then
