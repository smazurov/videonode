...
type WHEPOfferInput struct {
	...
	// clientIP is captured from the connection during Resolve; it is not part
	// of the request schema.
	clientIP string
}

// Resolve captures the client's socket address (host only) so the WebRTC peer
// can be labeled with it for logging and the consumers UI.
func (i *WHEPOfferInput) Resolve(ctx huma.Context) []error {
	i.clientIP = hostOnly(ctx.RemoteAddr())
	return nil
}
...
func registerWHEP(api huma.API, webrtcManager *WebRTCManager) {
	huma.Register(api, huma.Operation{
		...
		sessionID, answer, err := webrtcManager.CreateConsumer(input.Stream, string(input.RawBody), input.clientIP)
		if err != nil {
