...
// testReaderGoneDebounce shortens the last-reader-gone window so debounce
// tests run in milliseconds rather than the production default.
const testReaderGoneDebounce = 300 * time.Millisecond

func newTestServer(t *testing.T) *Server {
	t.Helper()
	s := NewServer(logging.GetLogger("streaming-test"))
	s.readerGoneDebounce = testReaderGoneDebounce
	return s
}
...
func TestLastReaderGone_FiresAfterDebounce(t *testing.T) {
	...
	// Should NOT have fired yet — still inside the debounce window.
	time.Sleep(testReaderGoneDebounce / 3)
	if fired.Load() != 0 {
...
func TestLastReaderGone_CancelledByReconnect(t *testing.T) {
	...
	// Reconnect well before the debounce expires; EnsureStreamReady
	...
	time.Sleep(2 * testReaderGoneDebounce)
	if n := fired.Load(); n != 0 {
