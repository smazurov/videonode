...
12. **Frame by capability, not by machine.** VideoNode is an open ecosystem: it runs on any Linux host with a GPU and is only currently tuned to Rockchip rk35xx. Never use deployment-machine jargon (`rig`, `dev box`, `the SBC`) or write prose that pins a concept to one SoC as if it were the only target. Describe code paths by capability (hardware-accelerated vs portable software) and name specific hardware (Rockchip MPP/RGA, `rk_hdmirx`) only as *today's* implementation, phrased so a future backend is an additive edit, not a rewrite. Exception: concrete operational pages (install steps, encoder validation) name the actually-shipped hardware where that is the literal supported target.
