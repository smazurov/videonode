...
TEST(SourceProbe, UvcWatchdogStallThenRecover) {
    v4l2::Streamer cap;
    SourceProbe p(cap);
    using clock = std::chrono::steady_clock;
    const auto deadline = std::chrono::milliseconds(200);
    p.set_stall_deadline(deadline);
    const clock::time_point t0{};
    p.note_dqbuf_success(t0);
    EXPECT_EQ(int(Health::Live), int(p.health()));
    p.note_tick(t0 + deadline + std::chrono::milliseconds(1));
    EXPECT_EQ(int(Health::NoLock), int(p.health()));
    p.note_dqbuf_success(t0 + deadline + std::chrono::milliseconds(50));
    EXPECT_EQ(int(Health::Live), int(p.health()));
}

TEST(SourceProbe, UvcWatchdogUnderDeadlineStaysLive) {
    v4l2::Streamer cap;
    SourceProbe p(cap);
    using clock = std::chrono::steady_clock;
    const auto deadline = std::chrono::milliseconds(200);
...
TEST(SourceProbe, HdmiWatchdogDoesNotForceNoLock) {
    v4l2::Streamer cap;
    SourceProbe p(cap);
    using clock = std::chrono::steady_clock;
    p.note_dv_timings_present(v4l2::Streamer::DvTimingsState::Locked);
    p.set_stall_deadline(std::chrono::milliseconds(100));
    const clock::time_point t0{};
    p.note_dqbuf_success(t0);
    EXPECT_EQ(int(Health::Live), int(p.health()));
    // DV timings stays Locked through a long frame gap: watchdog must not flip.
    p.note_tick(t0 + std::chrono::seconds(5));
    EXPECT_EQ(int(Health::Live), int(p.health()));
}

TEST(SourceProbe, ContentDeadGoesNoLockThenRecovers) {
    v4l2::Streamer cap;
    SourceProbe p(cap);
    p.note_dqbuf_success({});
    EXPECT_EQ(int(Health::Live), int(p.health()));
    p.note_content_dead(true);
    EXPECT_EQ(int(Health::NoLock), int(p.health()));
    p.note_content_dead(false);
    EXPECT_EQ(int(Health::Live), int(p.health()));
}
