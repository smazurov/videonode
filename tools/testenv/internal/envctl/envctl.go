...
// ReleaseWorktreeTree releases every env whose worktree is dir or an ancestor of
// dir (i.e. dir is the worktree root or a subdirectory of it). It backs the
// ExitWorktree PreToolUse intercept, where the reported cwd may be nested under
// the worktree root. Because signalDaemon blocks until each daemon exits, the
// worktree's files are released by the time this returns.
func ReleaseWorktreeTree(statePath, dir string) ([]string, error) {
	if dir == "" {
		return nil, nil
	}
	s, err := openStore(statePath)
	...
	var released []string
	for _, e := range envs {
		if worktreeContains(e.OwnerWorktree, dir) {
			signalDaemon(e.OwnerPID)
			if s.DeleteEnv(e.ID) == nil {
				removeSpawnFiles(e.OwnerWorktree, e.DataDir)
				released = append(released, e.ID)
...
// worktreeContains reports whether dir is the worktree owner itself or a
// subdirectory of it. Requiring a path separator after the owner prefix avoids
// false matches on sibling worktrees that share a name prefix (".../wt" must not
// match ".../wt-2").
func worktreeContains(owner, dir string) bool {
	if owner == "" || dir == "" {
		return false
	}
