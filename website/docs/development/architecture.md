...
The `StreamStatus` stream is the only long-lived RPC. It replaces a legacy polling watchdog: the source pushes a `Status` frame on health change, consumer-count change, or once per second as a heartbeat. The daemon's `pipelinectl.Manager` pumps these onto a buffered channel for the SSE fan-out path.
...
flowchart LR
  ...
  subgraph daemon["videonode (Go daemon)"]
    api["HTTP API"]
    mgr["pipelinectl.Manager"]
    api --> mgr
  end

  subgraph native["native binaries (C++)"]
    src["videonode-source"]
    comp["videonode-composer"]
    sink["videonode-sink"]
  end

  mgr -- "gRPC / Unix socket" --> src
  mgr -- "gRPC / Unix socket" --> comp

  src -- "SCM_RIGHTS NV12 dma-bufs" --> comp
  src -- "SCM_RIGHTS NV12 dma-bufs" --> sink

  sink -- "stdout NV12" --> ffmpeg["ffmpeg (encoder)"]
  comp -- "stdout BGRA" --> ffmpeg

  ffmpeg --> rtsp["RTSP / SRT / WebRTC"]
```
