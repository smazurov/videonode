...
func (c *HookPreToolUseCmd) Run(ctx *Context) error {
	...
	// ExitWorktree(remove) deletes the worktree via git, but an env running in
	// that worktree holds files in the tree and blocks the removal — and the
	// post-removal WorktreeRemove hook fires too late to help. Release the env
	// here, before the tool runs: signalDaemon blocks until the daemon exits, so
	// the tree is free by the time git tries to remove it.
	if p.ToolName == "ExitWorktree" {
		var in struct {
		...
		hookLog(ctx.StatePath, "pre-tool-use", ctx.SessionID,
			"tool", p.ToolName, "released", strconv.Itoa(len(released)))
		return nil
	}
