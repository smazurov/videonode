...
- **Empty page in dev?** `nodeLinker: hoisted` is missing in `pnpm-workspace.yaml`. Mermaid's transitives (dayjs, cytoscape, debug, `@braintree/sanitize-url`) need to be visible to Vite's `optimizeDeps` pre-bundler — strict pnpm hides them.
- **`pnpm build` failing on a dead link?** Fix the link, don't disable the `ignoreDeadLinks: false` guard.
- **Don't touch `docs/public/`** unless adding a binary asset (SVGs, screenshots). Files there ship at site root.
- **Deploys:** `.github/workflows/docs.yml` pushes `dist/` to `gh-pages` with `clean-exclude: dists pool public.key CNAME` so the apt repo survives. **Never add `force: true` or `force_orphan: true`** — they silently nuke the other workflow's content.
- **One concurrency group across both workflows:** `pages-deploy`. Don't change it.
