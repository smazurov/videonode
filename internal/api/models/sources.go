...
type SourceFormatBody struct {
	...
	// ColorMatrix is read-only: the YCbCr matrix the source binary detected
	// for the live stream, not an operator-selected field. Ignored on write.
	ColorMatrix string `json:"color_matrix,omitempty" example:"bt709" doc:"Detected YCbCr matrix (bt601/bt709); read-only, empty until the first status frame"`
}
