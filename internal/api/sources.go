...
type SourceFormat struct {
	...
	Height uint32
	FPS    uint32
	// ColorMatrix is the detected YCbCr matrix ("bt601"/"bt709"), live
	// status from the source binary; empty when not yet known. Read-only.
	ColorMatrix string
}
	...
	if src.Format != nil {
		...
		if ok {
			out.Format = &models.SourceFormatBody{
				...
				FPS:         src.Format.FPS,
				ColorMatrix: src.Format.ColorMatrix,
			}
		}
	}
	return out
