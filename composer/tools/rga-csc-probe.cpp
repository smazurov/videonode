...
// fill_bgr3_solid fills `buf` (size = w*h*3) with a solid BGR pixel.
...
uint8_t expected_nv12_luma_bt601(uint8_t b, uint8_t g, uint8_t r) {
    // full-range Y first (BT.601 coefficients)
    int fr = (299 * r + 587 * g + 114 * b) / 1000;
    // scale to limited range: Y = 16 + (219/255) * fr
...
bool check_solid_color(int W, int H, uint8_t b, uint8_t g, uint8_t r) {
    ...
    // Fill source with the solid color.
    ...
    // Inspect the destination luma plane.
