...
func (p *stubComposerPipeline) ApplyComposer(_ pipeline.Composer) error {
	p.applyCalls++
	return p.applyErr
}
...
func (p *stubComposerPipeline) UpdateComposerCanvas(_ string, _ pipeline.CanvasDims) error {
	p.canvasCalls++
	return p.canvasErr
}
...
// TestComposerService_Update_BackgroundHotApplies verifies a background-only
// canvas patch recolors live via UpdateComposerCanvas without restarting the
// process (ApplyComposer) or bouncing downstream encoders.
func TestComposerService_Update_BackgroundHotApplies(t *testing.T) {
	store := newStubStore()
	seedComposer(t, store)
	pipe := &stubComposerPipeline{}
	svc := newComposerSvc(store, pipe)

	patch := models.ComposerUpdateRequestData{
		Canvas: &models.CanvasDimsData{W: 1920, H: 1080, Background: "#1a1a2e"},
	}
	out, err := svc.UpdateComposer(context.Background(), "comp-1", patch)
	if err != nil {
		t.Fatalf("UpdateComposer returned error, want success: %v", err)
	}
	if out.Canvas.Background != "#1a1a2e" {
		t.Errorf("returned background = %q, want #1a1a2e", out.Canvas.Background)
	}
	if pipe.canvasCalls != 1 {
		t.Errorf("UpdateComposerCanvas calls = %d, want 1 (live recolor)", pipe.canvasCalls)
	}
	if pipe.applyCalls != 0 {
		t.Errorf("ApplyComposer calls = %d, want 0 (no restart for color change)", pipe.applyCalls)
	}
	if got, _ := store.GetComposerEntity("comp-1"); got.Canvas.Background != "#1a1a2e" {
		t.Errorf("persisted background = %q, want #1a1a2e", got.Canvas.Background)
	}
}

// TestComposerService_Update_DimsRestart verifies an actual canvas resize
// still takes the restart path (ApplyComposer), not the live recolor path.
func TestComposerService_Update_DimsRestart(t *testing.T) {
	store := newStubStore()
	seedComposer(t, store)
	pipe := &stubComposerPipeline{}
	svc := newComposerSvc(store, pipe)
	...
	if _, err := svc.UpdateComposer(context.Background(), "comp-1", patch); err != nil {
		t.Fatalf("UpdateComposer returned error, want success: %v", err)
	}
	if pipe.applyCalls != 1 {
		t.Errorf("ApplyComposer calls = %d, want 1 (resize restarts)", pipe.applyCalls)
	}
	if pipe.canvasCalls != 0 {
		t.Errorf("UpdateComposerCanvas calls = %d, want 0 (resize is not a live recolor)", pipe.canvasCalls)
	}
}
...
// TestComposerService_ReplaceLayout_SwitchOffPersistsWithoutPipelineCall
// ensures that with the master switch off, a layout edit persists to the
// store and makes NO pipeline call at all — the switch gates only native
// actions, and the pipeline reads the new layout through the store on its
// next spawn.
func TestComposerService_ReplaceLayout_SwitchOffPersistsWithoutPipelineCall(t *testing.T) {
	store := newStubStore()
	seedComposer(t, store)
	pipe := &stubComposerPipeline{}
	svc := &composerService{
