...
class SourceProbe {
  public:
    ...
    void note_dv_timings_present(v4l2::Streamer::DvTimingsState s);
    void note_dqbuf_success(std::chrono::steady_clock::time_point now);
    void note_dqbuf_failure(int errno_val);

    // note_tick drives the UVC silent-stall watchdog: a UVC dongle that loses
    // signal stops delivering buffers with no error to count, so only elapsed
    // time can flag it. No-op on the HDMI path (DV timings is the truth there).
    void note_tick(std::chrono::steady_clock::time_point now);

    void set_stall_deadline(std::chrono::steady_clock::duration d) { stall_deadline_ = d; }
    [[nodiscard]] static std::chrono::steady_clock::duration stall_deadline_for_fps(uint32_t fps);

    // note_content_dead records the UVC no-signal content verdict (flat +
    // frozen luma): a dongle that loses its input keeps streaming, so frames
    // flow but are dead. When set, health() degrades a live UVC source to
    // NoLock so the placeholder engages. No-op on the HDMI path.
    void note_content_dead(bool dead) { content_dead_ = dead; }
  ...
  private:
    ...
    std::chrono::steady_clock::duration stall_deadline_{};
    bool stalled_ = false;
    bool content_dead_ = false;
};
