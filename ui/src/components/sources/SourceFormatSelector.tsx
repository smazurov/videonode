...
import { aspectRatioLabel } from '../../lib/resolution';
...
export function SourceFormatSelector({
  ...
  useEffect(() => {
    ...
    if (sortedResolutions.length === 0) return;
    const match = sortedResolutions.find(
      (r) => r.width === value.width && r.height === value.height,
    );
    if (match) return;
    const first = sortedResolutions[0];
    if (!first) return;
    onChange({ format_name: formatName, width: first.width, height: first.height, fps: 0 });
  }, [loadingResolutions, formatName, sortedResolutions, value.width, value.height, onChange]);
    ...
    <div className="grid gap-3 sm:grid-cols-3">
      ...
      <div>
        ...
        <select
          ...
          className={selectClasses}
          disabled={disabled || loadingResolutions || sortedResolutions.length === 0}
          value={resolutionKey}
          onChange={(e) => {
        ...
        >
          {sortedResolutions.length === 0 && <option value="">—</option>}
          {sortedResolutions.map((r) => {
            const ar = aspectRatioLabel(r.width, r.height);
            return (
