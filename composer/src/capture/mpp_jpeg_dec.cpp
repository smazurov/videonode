...
jpeg_dec::Colorimetry colorimetry_of(MppFrameColorSpace spc) {
    switch (spc) {
    case MPP_FRAME_SPC_BT709:
        return jpeg_dec::Colorimetry::Bt709;
    case MPP_FRAME_SPC_FCC:
    case MPP_FRAME_SPC_BT470BG:
    case MPP_FRAME_SPC_SMPTE170M:
    case MPP_FRAME_SPC_SMPTE240M:
        return jpeg_dec::Colorimetry::Bt601;
    default:
        return jpeg_dec::Colorimetry::Unspecified;
    }
}
...
MppFrameColorSpace FrameRef::color_space() const {
    return frame_ ? mpp_frame_get_colorspace(frame_) : MPP_FRAME_SPC_UNSPECIFIED;
}
...
bool MppJpegDec::decode(std::span<const uint8_t> jpeg, jpeg_dec::DecodedNv12& out) {
    ...
    out.uv_offset = hs * vs;
    out.ver_stride = vs;
    out.colorimetry = colorimetry_of(f.color_space());
    ...
    case MPP_FMT_YUV444SP:
        ...
        break;
    default:
        // Reject unknown subsampling rather than mislabel it NV12 (the red-ghost bug).
        vn::log::error("mpp_jpeg_dec: unsupported decoded format %s, dropping frame",
                       mpp_fmt_name(f.fmt()));
        return false;
    }
