// Tests for vn::snapshot — MmapAndPack pitch handling, LatestFrameHolder
// round-trips for NV12 and BGRA, and concurrent producer/consumer.
...
// Expected tight-packed counterpart of fill_padded_'s output.
...
TEST(LatestFrameHolder, RoundTripNv12PaddedPitch) {
    // Padded pitch on both planes; tight-packed output must strip padding.
...
TEST(LatestFrameHolder, ConcurrentUpdateAndSnapshot) {
    // Two threads: producer Updates monotonically increasing frame_idx
    // against a stable fd; consumer Snapshots and asserts that each
    // returned frame_idx matches what the producer must have stashed
    // (monotonic, non-negative, within range). Repeat enough times that
    // any obvious race would surface.
