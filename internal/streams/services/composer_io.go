...
// write is the source of truth; the pipeline mirrors it (full re-apply when
// the switch is on, registry refresh otherwise).
...
// Switch off → registry refresh only (the process reloads from the store on
// its next spawn). Switch on → full re-apply (restart), rolling the store
// back to the prior state on rejection so a bad import is not persisted; a
// canvas-dimension change also bounces dependent encoders so their launch-time
// ffmpeg `-s` tracks the new broadcast size.
