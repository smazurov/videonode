...
struct LoopState {
    ComposeState cs;
    scm_rights_producer::ScmRightsProducer* scm_out = nullptr;
...
bool handle_not_ready_(LoopState& ls, const Snapshot& snap, int target_fps, RenderStats* stats,
    ...
    if (!ls.scm_out) {
        ...
        if (!ls.cs.compose->render({}, snap.background_rgba))
            return false;
...
bool ensure_compose_(LoopState& ls, std::atomic<bool>& running, const Snapshot& snap,
    ...
    if (++ls.init_attempts < kCanvasInitMaxAttempts) {
        ls.next_tick += kCanvasInitRetryDelay;
        return false;
    }
...
bool submit_frame_(LoopState& ls, const Snapshot& snap,
    ...
    if (!ls.cs.compose->render(ls.pending_batch.slots, snap.background_rgba)) {
